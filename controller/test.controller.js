...
class TestController {
  async getTestsInfo(req, res) {
    ...
    res.header("Content-Type", "application/json").status(200).send(JSON.stringify(tests.rows[0]));
  }
  ...
  async getUserPlayedAllTests(req, res) {
    ...
    res.header("Content-Type", "application/json").status(200).send(JSON.stringify(user_tests.rows[0]));
  }
